...
PP_test_30=predict(fit,newdata =test_30)
PP_test_30=round(PP_test_30,1)
class(PP_test_30)

plot(test_30$Price,PP_test_30)

res=test_30$Price- PP_test_30 #(real value-predicted value)
#root mean square error is as follows
RMSE_test_30=sqrt(mean(res^2))
RMSE_test_30
...
library(ggplot2)
d=data.frame(real=test_30$Price,predicted=PP_test_30)
ggplot(d,aes(x=real,y=predicted))+geom_point()

plot(fit,which = 1) #gives residual vz fitted plot
plot(fit,which = 2) #gives q-q-plot
plot(fit,which = 3) #gives scale-location plot
plot(fit,which = 4) #gives cooks distance

#check results for final test data
PP_test_final=predict(fit,newdata =test)
PP_test_final=round(PP_test_final,1)
class(PP_test_final)

#Writing preds

write.csv(PP_test_final, "Abdeali_Mithaiwala_P1_part2.csv",row.names=FALSE)
